...
<AccordionGroup>
  <Accordion title="How do I authenticate with the Python SDK?">
    Initialize <code>Client(api_key="YOUR_API_KEY")</code> for Pro features and higher limits. Free usage works with <code>Client()</code> for public endpoints.
  </Accordion>
  ...
  <Accordion title="How do I fetch historical OHLCV?">
    Use <code>client.ohlcv(coin_id, start=..., end=...)</code> with an API key. See the OHLCV example above.
  </Accordion>
