...
<Warning>
  The key is the entire header value. There is no `Bearer` prefix, and no other
  scheme word: `Authorization: Bearer YOUR_API_KEY` does not reach the API. Most
  APIs expect the opposite, so if your HTTP client has a "bearer token" field,
  set a raw header instead.
</Warning>
